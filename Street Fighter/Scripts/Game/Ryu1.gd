...
func mou():
	...
	velocitat.x = 0
	if Input.is_action_pressed('Right 1') and not Input.is_action_pressed('Punch 1'):
		velocitat.x = velocitat_maxima
	if Input.is_action_pressed('Left 1') and not Input.is_action_pressed('Punch 1'):
		velocitat.x = -velocitat_maxima
...
#			$AnimatedSprite.animation = "quiet"
	...
	if Input.is_action_pressed('Punch 1') and is_on_floor():
	...
	if Input.is_action_pressed("Kick 1") and is_on_floor():
#		$AnimatedSprite.animation = "patada"
	...
	if pot_disparar:
		if Input.is_action_pressed("Kami 1") and is_on_floor():
			pegant = true
